...
Vehicle::choose_next_state(map<int, vector<Vehicle>> predictions) {
  ...
  ***Here you can implement the transition_function code from the Behavior
  ...
  predicted vehicle trajectories as values. A trajectory is a vector of Vehicle
  objects. The first item in the trajectory represents the vehicle at the
  current timestep. The second item in the trajectory represents the vehicle one
  timestep in the future. OUTPUT: The the best (lowest cost) trajectory for the
  ego vehicle corresponding to the next ego vehicle state.

  Functions that will be useful:
  1. successor_states() - Uses the current state to return a vector of possible
  successor states for the finite state machine. 2. generate_trajectory(string
  state, map<int, vector<Vehicle>> predictions) - Returns a vector of Vehicle
  objects representing a vehicle trajectory, given a state and predictions. Note
  that trajectory vectors might have size 0 if no possible trajectory exists for
  the state. 3. calculate_cost(Vehicle vehicle, map<int, vector<Vehicle>>
  predictions, vector<Vehicle> trajectory) - Included from cost.cpp, computes
  the cost for a trajectory.
  */
  ...
  // bool set = false;
  // float lowest_cost = -1;
  // string optimal_success_state;
  // vector<Vehicle> optimal_trajectory;
  ...
  //   if (!set || (cost_for_state < lowest_cost)) {
  //     set = true;
  //     lowest_cost = cost_for_state;
  //     optimal_success_state = possible_successor_state;
  //     optimal_trajectory = trajectory_for_state;
  //   }
  // }
...
vector<Vehicle>
Vehicle::keep_lane_trajectory(map<int, vector<Vehicle>> predictions) {
  /*
