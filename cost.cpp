...
float goal_distance_cost(const Vehicle &vehicle,
  ...
  larger as vehicle approaches goal distance.
  */
  float cost;
...
float calculate_cost(const Vehicle &vehicle,
  ...
  */
  ...
  vector<
      function<float(const Vehicle &, const vector<Vehicle> &,
                     const map<int, vector<Vehicle>> &, map<string, float> &)>>
...
get_helper_data(const Vehicle &vehicle, const vector<Vehicle> &trajectory,
  ...
  indended_lane: the current lane +/- 1 if vehicle is planning or executing a
  lane change. final_lane: the lane of the vehicle at the end of the trajectory.
